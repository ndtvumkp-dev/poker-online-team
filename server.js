...
function publicState(room, viewerId=null) {
  ...
  const players = room.players
    ...
    .map(p => ({
      ...
      cards: (viewerId && p.id === viewerId) ? p.cards : (p.cards.length ? ["??","??"] : [])
    }));
...
function nextSeat(room, fromSeat) {
  const alive = room.players.filter(p => p.tokens > 0 || p.betThisRound > 0 || p.cards.length > 0);
...
function resolveShowdown(room) {
  ...
  for (const w of winners) {
    w.tokens += share;
    if (remainder > 0) { w.tokens += MIN_UNIT; remainder -= MIN_UNIT; } // chia lẻ theo unit 5
  }

  room.pot = 0;

  // (tùy chọn) sau showdown, tự động bắt đầu ván mới sau 3 giây
  broadcastRoom(room);
  setTimeout(() => {
    // chỉ start nếu còn >=2 người có token
...
function doAction(room, player, action, amount=0) {
  ...
  if (action === "fold") {
    ...
    if (toCall !== 0) return; // không được check khi đang bị bet
  } else if (action === "call") {
    takeFromPlayer(room, player, toCall);
  } else if (action === "bet") {
    // bet chỉ hợp lệ nếu chưa có ai bet (highestBet=0)
    if (room.highestBet !== 0) return;
    if (amount < MIN_UNIT) return;
    takeFromPlayer(room, player, amount);
  } else if (action === "raise") {
    // raise: trả toCall + thêm amount (amount là phần raise thêm)
    if (room.highestBet === 0) return;
    if (amount < MIN_UNIT) return;
  ...
  // Move turn / advance stage if settled
  // Nếu chỉ còn 1 người => end
  const act = activePlayers(room);
  if (act.length === 1) {
    act[0].tokens += room.pot;
    room.pot = 0;
...
io.on("connection", (socket) => {
  socket.on("join", ({ roomId, name }) => {
    ...
    // gửi state riêng cho người này + broadcast cho cả phòng
    broadcastRoom(room);
